...
function getShareBriefLabel(): string {
  return SITE_VARIANT === 'media' ? 'Story Brief' : 'Intelligence Brief';
}

function getShareHeadlineLabel(): string {
  return SITE_VARIANT === 'media' ? 'Story Update' : 'Intelligence Update';
}
...
export const shareTexts = {
  twitter: (data: StoryData) =>
    `${toFlagEmoji(data.countryCode, '')} ${data.countryName} ${getShareBriefLabel()}\n\n` +
  ...
  linkedin: (data: StoryData) =>
    `${getShareHeadlineLabel()}: ${data.countryName}\n\n` +
  ...
  telegram: (data: StoryData) =>
    `${toFlagEmoji(data.countryCode, '')} *${data.countryName} ${SITE_VARIANT === 'media' ? 'Story Desk' : 'Intelligence'}*\n\n` +
    `📊 Instability: *${data.cii?.score || 'N/A'}/100* (${data.cii?.level || 'N/A'})\n` +
    `📈 Trend: *${data.cii?.trend || 'stable'}*\n` +
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  const shareBriefLabel = getShareBriefLabel();
  
  return {
    twitter: `https://twitter.com/intent/tweet?text=${text}&url=${encodeURIComponent(url)}`,
    linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`,
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} ${shareBriefLabel} - ${shareBrand}`)}`,
